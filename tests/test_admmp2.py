import unittest
from src.algorithme.admmp2 import ADMMP2_v2 as ADMMP2
import numpy as np
...
class Test_TestADMMP2(unittest.TestCase):
    def test_gaussian_filter_2d(self):
        ...
        self.assertTrue(np.allclose(np.asarray(y.shape), np.asarray(x.shape) - h.shape[0]+1))
    ...
    def test_precompute_matrix_padding(self):
        ...
        algo = ADMMP2(x, h)
        y =  algo.gaussian_filter_2d()
        pre_comput_Ty =  algo._precompute_matrix(y)
    ...
    def test_adjoint_R_2(self):
        ...
        algo = ADMMP2(x, h)
        Rx = algo.wavelet_transform_2(x)
        y = rng.integers(0, 255, size=Rx.shape) / 255.
        R_star_y = algo.wavelet_transform_adjoint_2(y, int(Rx.shape[0]//2))
        dot_a = np.hstack(Rx) @ np.hstack(y)
        dot_b = np.hstack(x) @ np.hstack(R_star_y)
        self.assertAlmostEqual(dot_a, dot_b)